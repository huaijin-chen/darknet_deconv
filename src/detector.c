...
void train_detector(char *datacfg, char *cfgfile, char *weightfile, int *gpus, int ngpus, int clear)
{
    ...
    while(get_current_batch(net) < net.max_batches){
        if(l.random && count++%10 == 0){
            ...
            int dim = (rand() % 17 + 3) * 32;
            if (*net.seen < 4000*net.batch*ngpus){
                printf("Scale 13 ~ 17, odd\n");
                dim = (rand() % 3 * 2 + 13) * 32;
                //dim = 96;
            }
            else if(*net.seen < 8000*net.batch*ngpus){
                printf("Scale 9 ~ 17, odd\n");
                dim = (rand() % 5 * 2 + 9) * 32;
            }
            else if(*net.seen < 12000*net.batch*ngpus){
                printf("Scale 5 ~ 17, odd\n");
                dim = (rand() % 7 * 2 + 5) * 32;
            }
            else if(*net.seen < 16000*net.batch*ngpus){
                printf("Scale 5 ~ 17, all\n");
                dim = (rand() % 13 + 5) * 32;
            }
            else if(*net.seen < 20000*net.batch*ngpus){
                printf("Scale 3 ~ 19, all\n");
                dim = (rand() % 17 + 3) * 32;
            }
            else{
                printf("Scale 2 ~ 19, all\n");
                dim = (rand() % 18 + 2) * 32;
            }
...
void validate_detector_recall(char *cfgfile, char *weightfile, char * listfile)
{
    network net = parse_network_cfg(cfgfile);
    if(weightfile){
        load_weights(&net, weightfile);
    ...
    for(i = 0; i < m; ++i){
        ...
        float precision = 1.0*correct/proposals;
        float recall = 1.0*correct/total;
        float fscore = 2*precision*recall/(precision+recall);
        fprintf(stderr, "%5d %5d %5d\tRPs/Img: %.2f\tIOU: %.2f%%\tRecall: %.2f%%\tPrecision: %.2f%%\tFscore: %.2f%%\n", i, correct, total, (float)proposals/(i+1), avg_iou*100/total, 100.*recall, 100.*precision, 100.*fscore);
        free(id);
        free_image(orig);
        free_image(sized);
    }
}
void test_detector_list(char *datacfg, char *cfgfile, char *weightfile, char *filelist, float thresh)
{
    list *options = read_data_cfg(datacfg);
    char *name_list = option_find_str(options, "names", "data/names.list");
    char **names = get_labels(name_list);
...
void eval_detector_list(char *cfgfile, char *weightfile, char *filelist)
{
    network net = parse_network_cfg(cfgfile);
    if(weightfile){
        load_weights(&net, weightfile);
...
void run_detector(int argc, char **argv)
{
    ...
    else if(0==strcmp(argv[2], "testlist")) test_detector_list(datacfg, cfg, weights, filename, thresh);
    else if(0==strcmp(argv[2], "evallist")) eval_detector_list(cfg, weights, filename);
    else if(0==strcmp(argv[2], "train")) train_detector(datacfg, cfg, weights, gpus, ngpus, clear);
    else if(0==strcmp(argv[2], "valid")) validate_detector(datacfg, cfg, weights);
    else if(0==strcmp(argv[2], "recall")) validate_detector_recall(cfg, weights, filename);
