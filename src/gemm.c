...
void gemm_tn(int M, int N, int K, float ALPHA, 
        ...
        float *C, int ldc)
{
    //TA=1 TB=0 M=18 N=9 K=3 ALPHA0.000000 lda=1 ldb=18 BETA=0.0 ldc=9
    /*
     *  
     */
    int i,j,k;
    for(i = 0; i < M; ++i){ // m = 18 
        for(k = 0; k < K; ++k){ // k=3
            register float A_PART = ALPHA*A[k*lda+i]; //lda = 1 a-->1x3 
            for(j = 0; j < N; ++j){ //n=9
                C[i*ldc+j] += A_PART*B[k*ldb+j]; //ldb=18 ldc=9 b-->18x9 b-->3x9
            }
        }
    }
}
