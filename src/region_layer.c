...
region_layer make_region_layer(int batch, int w, int h, int n, int classes, int coords)
{
    ...
    l.biases = calloc(n*4, sizeof(float));
    l.bias_updates = calloc(n*4, sizeof(float));
...
box get_region_box(float *x, float *biases, int n, int index, int i, int j, int w, int h)
{
    ...
    b.w = exp(x[index + 2]) * biases[4*n];
    b.h = exp(x[index + 3]) * biases[4*n+1];
    if(DOABS){
        b.w = exp(x[index + 2]) * biases[4*n]   / w;
        b.h = exp(x[index + 3]) * biases[4*n+1] / h;
    }
    return b;
}
...
float delta_region_box(box truth, float *x, float *biases, int n, int index, int i, int j, int w, int h, float *delta, float scale)
{
    ...
    float tw = log(truth.w / biases[4*n]);
    float th = log(truth.h / biases[4*n + 1]);
    if(DOABS){
        tw = log(truth.w*w / biases[4*n]);
        th = log(truth.h*h / biases[4*n + 1]);
    }
    ...
    for (b = 0; b < l.batch; ++b) {
            ...
            for (i = 0; i < l.w; ++i) {
                for (n = 0; n < l.n; ++n) {
                    ...
                    if(*(state.net.seen) < 12800){
                        box truth = {0};
                        truth.x = (i + l.biases[4*n+2])/l.w;
                        truth.y = (j + l.biases[4*n+3])/l.h;
                        truth.w = l.biases[4*n];
                        truth.h = l.biases[4*n+1];
                        if(DOABS){
                            truth.w = l.biases[4*n]/l.w;
                            truth.h = l.biases[4*n+1]/l.h;
                        }
        ...
        for(t = 0; t < 30; ++t){
            ...
            for(n = 0; n < l.n; ++n){
                ...
                if(l.bias_match){
                    pred.w = l.biases[4*n];
                    pred.h = l.biases[4*n+1];
                    if(DOABS){
                        pred.w = l.biases[4*n]/l.w;
                        pred.h = l.biases[4*n+1]/l.h;
                    }
                }
                ...
                float dist = pow(((i+l.biases[4*n+2]) - truth.x * l.w), 2);
                dist = dist + pow(((j+l.biases[4*n+3]) - truth.y * l.h), 2);
                //printf("(%f,%f  %f, %f) (%f,%f,%f,%f)\n", best_iou, min_dist, iou, dist, l.biases[4*n],l.biases[4*n+1],l.biases[4*n+2],l.biases[4*n+3]);
                if (iou > best_iou){
                    best_index = index;
                    best_iou = iou;
                    best_n = n;
